...
class TelecomUserFactory(UserFactory):
    ...
    def __init__(self, dataset_dir: str, node_coordinates: np.array, functions: np.array):
        self.dataset_dir = pathlib.Path(dataset_dir)
        users = pd.read_excel(self.dataset_dir.joinpath("data_6.16.15.xlsx"),
...
if __name__ == '__main__':
    ...
    # user_factory = TelecomUserFactory("telecom", node_coordinates, functions)
    # a = [sum(user_factory.get_workload(t/n)) for t in range(1, n)]
    # print(f'{[int(k) for k in a]}')
    # print(len(a))
    # print(max(a))
    # pass