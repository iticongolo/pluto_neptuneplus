...
def constrain_network_delay(data, solver, x, coeff):
    ...
    vals = list(itertools.product(range(len(data.functions)),
                                  range(len(data.sources)),
                                  range(len(data.nodes))))

    solver.Add(
        solver.Sum([
            x[f, i, j] * float(data.node_delay_matrix[i, j] * data.workload_matrix[f, i])
            for f, i, j in vals
        ]) <= coeff * np.sum(
            [float(data.node_delay_matrix[i, j] * data.workload_matrix[f, i] * data.prev_x[f, i, j]) for f, i, j in
             vals])
    )
...
def constrain_score(data, solver, x, n, alpha):
    ...
    solver.Add(
        ...
        solver.Sum([x[f, i, j] * float(
            (1 - alpha) * data.workload_matrix[f, i] * data.node_delay_matrix[i, j] / max_delay_matrix[i, f]) for
                    f, i, j in
                    itertools.product(range(len(data.functions)), range(len(data.nodes)),
